...
ticker = 'AAPL'
df = yf.download(ticker, start='2000-01-01', group_by='ticker')

def create_dataset(df, close_col, volume_col, high_col, low_col):
    # Basic features
...
# Create Sequences
# ----------------------------
def create_windowed_dataset(df, ticker):
    window=200
    features = [
...
if __name__ == "__main__":
    ...
    if trainModel:
        ...
        df = create_dataset(df, close_col, volume_col, high_col, low_col)
        ...
        X, y = create_windowed_dataset(df_balanced, ticker)
        ...
        model.summary()
        model.save(f'/Users/admin/FinAi/regression_price_pred/{ticker}/{ticker}_model.keras')
        ...
        import daily_check
        import os
        base_path = '/Users/admin/FinAi/regression_price_pred'
        path = os.path.join(base_path, ticker)
        if os.path.basename(path) != ticker:
            raise ValueError(f"Ticker '{ticker}' does not match the last part of the path: '{os.path.basename(path)}'")
