...
# -*- coding: utf-8 -*-
"""
Fetch market data and split into train vs validation:
...
# 1) URLs for Wikipedia index pages
WIKI_URLS = {
...
# 2) ETF fallbacks if scrape fails
FALLBACK = {
    "sp500":       ["SPY"],
    "nasdaq100":   ["QQQ"],
    "dow_jones":   [
...
# 3) Crypto tickers we want in TRAIN
CRYPTOS = ["BTC-USD", "ETH-USD", "XRP-USD", "SOL-USD", "BNB-USD", "TRX-USD", "DOGE-USD"]

def scrape_index_tickers(idx_name):
    """Scrape the Wikipedia table for idx_name, fall back if anything fails."""
    url = WIKI_URLS[idx_name]
    try:
        ...
        for tbl in tables:
            ...
            # look for any ticker-like header
        ...
        return FALLBACK[idx_name]

def fetch_and_save(symbols, out_folder, start, end):
...
if __name__ == "__main__":
    ...
    VAL_IDX   = ["dow_jones", "russell2000", "ftse100", "dax", "nikkei225"]

    # 1) TRAIN: indices + cryptos
    train_syms = []
    for idx in TRAIN_IDX:
        train_syms += scrape_index_tickers(idx)
    train_syms += CRYPTOS  # add crypto
    print(f"\n=== TRAIN set: {len(train_syms)} symbols ===")
    fetch_and_save(train_syms, os.path.join(BASE, "train"), START, END)

    # 2) VALIDATION: the other indices
    val_syms = []
    for idx in VAL_IDX:
        val_syms += scrape_index_tickers(idx)
    print(f"\n=== VALIDATION set: {len(val_syms)} symbols ===")
    fetch_and_save(val_syms, os.path.join(BASE, "validation"), START, END)
